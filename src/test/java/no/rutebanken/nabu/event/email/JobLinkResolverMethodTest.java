...
class JobLinkResolverMethodTest {
    ...
    private final JobLinkResolverMethod jobLinkResolverMethod = new JobLinkResolverMethod(BASE_URL);
    ...
    private void assertLink(JobEvent jobEvent, String relativeExpectedLink) {
        try {
            Object link = jobLinkResolverMethod.exec(List.of(new BeanModel(jobEvent, new DefaultObjectWrapper(Configuration.VERSION_2_3_34))));
            if (relativeExpectedLink == null) {
                assertThat(link).isNull();
            } else {
                assertThat(link).isEqualTo((BASE_URL + relativeExpectedLink));
            }
