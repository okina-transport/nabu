...
public class SystemJobStatusRepositoryImplTest extends BaseIntegrationTest {
    ...
    public void testFind() {
       ...
       assertEquals(3, systemJobStatusRepository.find(null, null).size());

       assertEquals(Arrays.asList(s3), systemJobStatusRepository.find(Arrays.asList("dom2"), null));
       assertEquals(Arrays.asList(s2), systemJobStatusRepository.find(new ArrayList<>(), Arrays.asList("type2")));
       assertEquals(Arrays.asList(s1), systemJobStatusRepository.find(Arrays.asList("dom1"), Arrays.asList("type1")));
    }
}
