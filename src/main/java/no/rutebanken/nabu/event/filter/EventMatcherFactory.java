...
public class EventMatcherFactory {
    ...
    public EventMatcher createEventMatcher(EventFilterDTO eventFilter) {
        ...
        if (EventFilterDTO.EventFilterType.CRUD.equals(eventFilter.getType())) {
            return new CrudEventMatcher(administrativeZoneRepository, eventFilter);
        } else if (EventFilterDTO.EventFilterType.JOB.equals(eventFilter.getType())) {
            return new JobEventMatcher(eventFilter);
        }
        throw new NabuException("Unsupported eventFilter type: " + eventFilter.getType());
    }
}
